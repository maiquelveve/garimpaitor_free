...
import { CREATE_BRAND_SYSTEM } from '../../../../../graphql/brands/mutation';
...
export default function CreateBrand() {
  ...
  const [createBrandSystem, { loading }] = useMutation(CREATE_BRAND_SYSTEM, optionsCreateBrandSystem);
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        const { name } = values;
        const variables = { name, network_id: networkCurrent.id }
        const result = await createBrandSystem({ variables });
